...
namespace IP3D
{
    class SystemParticulaDustBoid
    {
        ...
        private ClsTankBoid tank;
        
        GraphicsDevice device;
        ClsTerreno terreno;

        // Sistema de particulas, lista de particulas 
        public SystemParticulaDustBoid(GraphicsDevice device, ClsTankBoid tank,ClsTerreno terreno)
        {
            r = new Random(1);
            this.tank = tank;
            this.device = device;
        ...
        {
            if (tank.actual== ClsTankBoid.tankState.move) Generate((int)(Math.Round(particlesPerSec * (float)gametime.ElapsedGameTime.TotalSeconds)));
